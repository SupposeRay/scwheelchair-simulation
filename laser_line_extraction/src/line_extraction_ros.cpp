...
void LineExtractionROS::populateLineSegListMsg(const std::vector<Line> &lines,
                                                scat_msgs::EnvObjectList &line_list_msg)
{
  for (std::vector<Line>::const_iterator cit = lines.begin(); cit != lines.end(); ++cit)
  {
    scat_msgs::EnvObject line_msg;
    line_msg.ID = 1;
    line_msg.params.push_back(cit->getStart()[0]);
    line_msg.params.push_back(cit->getStart()[1]);
    line_msg.params.push_back(0.0);
    line_msg.params.push_back(cit->getEnd()[0]);
    line_msg.params.push_back(cit->getEnd()[1]);
    line_msg.params.push_back(0.0);
    ...
    line_list_msg.objects.push_back(line_msg);
  }
  line_list_msg.header.frame_id = scan_frame_id_;
