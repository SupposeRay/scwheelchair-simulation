...
{
	loadParameters();
	line_publisher_ = nh_.advertise<scat_msgs::EnvObjectList>("/scan_obstacles", 1);
...
void LaserFeatureROS::publishMarkerMsg(const std::vector<gline> &m_gline,visualization_msgs::Marker &marker_msg)
{
	...
	marker_msg.header.frame_id = scan_frame_id_;
	marker_msg.header.stamp = ros::Time::now();
}

void LaserFeatureROS::publishLineMsg(const std::vector<gline> &m_gline, scat_msgs::EnvObjectList &line_msg)
{
	line_msg.header.frame_id = scan_frame_id_;
	line_msg.header.stamp = ros::Time::now();

	for (std::vector<gline>::const_iterator cit = m_gline.begin(); cit != m_gline.end(); ++cit)
	{
		scat_msgs::EnvObject line;
		line.ID = 1;
		line.params.push_back(cit->x1);
	    line.params.push_back(cit->y1);
	    line.params.push_back(0.0); //z
	    line.params.push_back(cit->x2);
	    line.params.push_back(cit->y2);
	    line.params.push_back(0.0); //z
		line_msg.objects.push_back(line);
	}
}
...
void LaserFeatureROS::startgame()
{
	...
	std::vector<gline> glines;
  	...
  	scat_msgs::EnvObjectList line_msg;
    publishLineMsg(glines, line_msg);
	line_publisher_.publish(line_msg);

}

// Load ROS parameters
void LaserFeatureROS::loadParameters()
{
	ROS_DEBUG("*************************************");
