...
namespace shared_DWA
{
    ...
    :   node_handle_(node_handle)
    {
        ...
        goal_publisher_ = node_handle_.advertise<visualization_msgs::Marker>("/visualization/doorway_marker", 1);
        timer_ = node_handle_.createTimer(ros::Duration(publish_interval), &shared_DWANode::timerCallback, this);
        // Service Client
        doorway_client_ = node_handle_.serviceClient<shared_dwa::doorway_srv>("/doorway_server");
        ...
        doorway_marker.header.frame_id = base_frame_id;
        doorway_marker.ns = "visualization";
        doorway_marker.action = visualization_msgs::Marker::ADD;
        doorway_marker.pose.orientation.w = 1.0;
        doorway_marker.id = 3;
        doorway_marker.type = visualization_msgs::Marker::ARROW;
        doorway_marker.scale.x = 0.03;
        doorway_marker.scale.y = 0.05;
        doorway_marker.scale.z = 0.0;
        doorway_marker.color.b = 1.0;
        doorway_marker.color.g = 1.0;
        doorway_marker.color.a = 1.0;
    }
    // Destructor
    ...
    void shared_DWANode::timerCallback(const ros::TimerEvent&)
    {
        ...
        if (cmd_receive && odom_receive && scan_receive)
        {
            ...
            else
            {
                ...
                // ROS_DEBUG_STREAM("Dynamic window generated.");
                shared_DWANode::selectVelocity();
                // ROS_DEBUG_STREAM("Velocity selected.");
            }
            publishResults();
        }
    }
    ...
    void shared_DWANode::dynamicWindow()
    {
        ...
        // doorway traversal
        float min_dist2goal, angle2goal;
        if (doorway_found)
        {
        ...
        for (int i = 0; i < v_sample + 1; i++)
        {
            for (int j = 0; j < w_sample + 1; j++)
            {
                ...
                min_dist2goal = 1000;
                angle2goal = 0;
                clearance_window.at<float>(i, j) = shared_DWANode::calDist2Collision(v_dw, w_dw, sample_interval, sample_time, min_dist2goal, angle2goal);
                if (doorway_found)
                {
                    // ROS_DEBUG_STREAM("min_dist2goal " << min_dist2goal);
                    // ROS_DEBUG_STREAM("angle2goal " << angle2goal);
    ...
    float shared_DWANode::calDist2Collision(float v_dw, float w_dw, float sample_interval, float sample_time, float& min_dist2goal, float& angle2goal)
    {
        // ROS_DEBUG_STREAM("v_dw " << v_dw << ", w_dw " << w_dw);
        float dist2collision;
        if (!agent_forward)
        {
            v_dw *= -1;
        }
        ...
        for (int i = 0; i < sample_number; i++)
        {
            ...
            if (shared_DWANode::checkCollision(x, y, theta, footprint_mode))
            {
                ...
                else
                {
                    ...
                    dist2collision =  static_cast<float>(i + 1) / static_cast<float>(sample_number);
            ...
            if (doorway_found)
            {
                temp_dist2goal = calDistance(x, y, doorway_goal.position.x, doorway_goal.position.y);
    ...
    float shared_DWANode::calHeading(float v_current, float w_current, float v_command, float w_command)
    {
        ...
        v_current = std::max( fabs(v_current / v_max_robot), double(0.0001));
    ...
    void shared_DWANode::selectVelocity()
    {
        if (window_generate)
        {
            ...
            // ROS_DEBUG_STREAM("Computing the cost window.");
            // ROS_DEBUG_STREAM("weight_cmd " << weight_cmd);
            // ROS_DEBUG_STREAM("weight_goal " << weight_goal);
            // ROS_DEBUG_STREAM("path_cost_window " << path_cost_window);
            // ROS_DEBUG_STREAM("heading_window " << heading_window);
            // ROS_DEBUG_STREAM("velocity_window " << velocity_window);
            // ROS_DEBUG_STREAM("clearance_window " << clearance_window);
            cost_window = weight_cmd * (weight_heading * heading_window + weight_velocity * velocity_window) + weight_goal * path_cost_window;
            cost_window = 1 - clearance_window + clearance_window.mul(cost_window);
            // ROS_DEBUG_STREAM("Cost window generated.");
            double min_cost = 0;
            int min_idx[2] = {};
    ...
    void shared_DWANode::generateGoal()
    {
        // ROS_INFO_STREAM("Calling doorway detection service");
        detection_srv.request.doorway_request = "doorway_request";
        if (doorway_client_.call(detection_srv))
        {
            if (detection_srv.response.find_door)
            {
                doorway_found = true;
                doorway_goal.position.x = detection_srv.response.point_x;
                doorway_goal.position.y = detection_srv.response.point_y;
                doorway_goal.position.z = detection_srv.response.point_z;
                doorway_goal.orientation.x = detection_srv.response.quat_x;
                doorway_goal.orientation.y = detection_srv.response.quat_y;
                doorway_goal.orientation.z = detection_srv.response.quat_z;
                doorway_goal.orientation.w = detection_srv.response.quat_w;
                tf::quaternionMsgToTF(doorway_goal.orientation, goal_quat);
                tfScalar yaw, pitch, roll;
                tf::Matrix3x3(goal_quat).getRPY(roll, pitch, yaw);
                goal_yaw = yaw;
                doorway_marker.points.clear();
                doorway_marker.points.push_back(doorway_goal.position);
                tf::Vector3 goal_direction(0.5, 0, 0);
                goal_direction = tf::Matrix3x3(goal_quat) * goal_direction;
                geometry_msgs::Point goal_end;
                goal_end.x = doorway_goal.position.x + goal_direction.getX();
                goal_end.y = doorway_goal.position.y + goal_direction.getY();
                goal_end.z = doorway_goal.position.z + goal_direction.getZ();
                doorway_marker.points.push_back(goal_end);
                ROS_DEBUG_STREAM("Doorway goal received.");
            }
        }
        // else
        // {
        ...
        //     tf::Matrix3x3 goal_rotation;
        //     geometry_msgs::Point goal_start, goal_end;

        //     goal_rotation.setRPY(0, 0, 0.5*PI);
        //     goal_yaw = 0.5*PI;
        //     goal_start.x = 1;
        //     goal_start.y = 0.5;
        //     goal_start.z = 0;
        //     doorway_marker.points.clear();
        //     doorway_marker.points.push_back(goal_start);
        //     doorway_goal.position = goal_start;
        //     tf::Vector3 goal_direction(0.8, 0, 0);
        //     goal_direction = goal_rotation * goal_direction;
        //     goal_end.x = doorway_goal.position.x + goal_direction.getX();
        //     goal_end.y = doorway_goal.position.y + goal_direction.getY();
        //     goal_end.z = doorway_goal.position.z + goal_direction.getZ();
        //     doorway_marker.points.push_back(goal_end);
        //     // ROS_DEBUG_STREAM("Dummy goal generated.");
        // }
    }
    cv::Mat shared_DWANode::calPathcost(cv::Mat dist_cost, cv::Mat angle_cost)
    {
        ...
        // ROS_DEBUG_STREAM("dist_cost_normalized " << dist_cost);
        // ROS_DEBUG_STREAM("angle_cost_normalized " << angle_cost);
