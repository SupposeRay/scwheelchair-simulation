...
namespace shared_DWA
{
    ...
    :   node_handle_(node_handle)
    {
        ...
        if (trajectory_listen)
        {
            traj_subscriber_ = node_handle_.subscribe("/path", 1, &shared_DWANode::trajCallback, this);
        }
        ...
        traj_line.header.frame_id = base_frame_id;
        traj_line.ns = "visualization";
    ...
    bool shared_DWANode::readParameters()
    {
        ...
        {
            trajectory_listen = false;
        }
        else if (guidance_mode == "enable")
        {
            trajectory_listen = true;
        }
        else
            return false;
    ...
    void shared_DWANode::odomCallback(const nav_msgs::Odometry &msg_odom)
    {
        ...
        // q_agent = msg_odom.pose.pose.orientation;
        tf::quaternionMsgToTF(msg_odom.pose.pose.orientation, q_agent);
    ...
    void shared_DWANode::trajCallback(const nav_msgs::Path &msg_traj)
    {
        path_traj = msg_traj;
        traj_line.header.stamp = ros::Time::now();
        traj_line.points.clear();
        for (int i = 0; i < path_traj.poses.size(); i++)
        {
            traj_line.points.push_back(path_traj.poses[i].pose.position);
        }
        traj_receive = true;
    }
    ...
    void shared_DWANode::dynamicWindow()
    {
        ...
        if (trajectory_listen)
        {
            traj_cost_window.create(v_sample + 1, w_sample + 1, CV_32F);
        }
    ...
    float shared_DWANode::calDist2Collision(float v_dw, float w_dw, float sample_interval, float sample_time)
    {
        ...
        if (trajectory_listen)
        {
            float min_dist2subgoal = 1000, temp_dist2subgoal = 0; angle2subgoal = 0; subgoal_angle = 0;
            tf::Matrix3x3 subgoal_rotation(subgoal_quat);
            subgoal_rotation.getRPY(NULL, NULL, subgoal_angle);
        }        
        for (int i = 0; i < sample_number; i++)
        {
            ...
            if (trajectory_listen)
            {
                temp_dist2subgoal = calDistance(x, y, subgoal_.position.x, subgoal_.position.y);
                if (temp_dist2subgoal <= min_dist2subgoal)
                {
                    min_dist2subgoal = temp_dist2subgoal;
                    angle2subgoal = fabs(theta - subgoal_angle);
                }
            }
    ...
    void generateSubgoal()
    {
        if (trajectory_listen)
        {
            if (odom_receive && traj_receive)
            {
                if (!path_traj.poses.empty())
                {
                    float dist_subgoal = 0;
                    for (int i = starting_point; i < path_traj.poses.size(); i++)
                    {
                        dist_subgoal = calDistance(x_agent, y_agent, path_traj.poses[i].pose.position.x, path_traj.poses[i].pose.position.y);
                        if (dist_subgoal < 0.5)
                        {
                            if (starting_point == path_traj.poses.size() - 1)
                            {
                                subgoal_ = path_traj.poses[starting_point].pose;
                                break;
                            }
                            else
                            {
                        ...
                        {
                            subgoal_ = path_traj.poses[i].pose;
                            break;
                        }
                    }
                    tf::quaternionMsgToTF(subgoal_.pose.orientation, subgoal_quat);
                }
                else
                {
                    ROS_ERROR_STREAM("The guiding path is empty!");
                }
            }
            else
            {
                ROS_ERROR_STREAM("Odometry or guiding path not received!");
            }
        }
    }
}
